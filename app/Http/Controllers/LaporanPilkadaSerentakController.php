...
class LaporanPilkadaSerentakController extends Controller
{
    ...
    private $storageDiskPath = 'laporan_pilkada_serentak/file'; // Path relatif di dalam disk
    private $publicAccessPath = 'storage/laporan_pilkada_serentak/file'; // Path untuk disimpan di DB
    ...
    {
        // Simpan ke 'public' disk di folder 'laporan_pilkada_serentak/file'
        // store() otomatis menghasilkan hashName
        $path = $file->store($this->storageDiskPath, $this->diskName);

        // $path akan berisi 'laporan_pilkada_serentak/file/hash.ext'
    ...
    private function deleteFile(?string $filePath): void
    {
        ...
        // $filePath dari DB adalah 'storage/laporan_pilkada_serentak/file/hash.ext'
        // Kita perlu 'laporan_pilkada_serentak/file/hash.ext' untuk dihapus
    ...
    {
        $laporan = LaporanPilkadaSerentak::with(['operator.wilayah'])->findOrFail($id);
        ...
        $filters = [
            'tipe_laporan' => 'Harian Pilkada Serentak',
        ...
        $pdf = Pdf::loadView('dashboard.laporan_pilkada_serentak.preview_pdf', compact(
            'laporan',
            'reports',
            'wilayah',
            'filters',
    ...
    {
        return view('dashboard.laporan_pilkada_serentak.index');
    }
    ...
    public function data()
    {
        ...
        $query = LaporanPilkadaSerentak::with('operator')
            ->latest('id_laporan');
        ...
        return datatables()
            ...
            ->addColumn('aksi', function ($laporan) {
                ...
                $editUrl = route('laporan_pilkada_serentak.edit', $laporan->id_laporan);
                $showUrl = route('laporan_pilkada_serentak.show', $laporan->id_laporan);
                $deleteUrl = route('laporan_pilkada_serentak.destroy', $laporan->id_laporan);
    ...
    {
        return view('dashboard.laporan_pilkada_serentak.create');
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            $validatedData['judul'] = 'Laporan Harian Pilkada Serentak';
            ...
            $laporanBaru = LaporanPilkadaSerentak::create($validatedData);
            ...
            try {
                ...
                if ($pimpinanUsers->isNotEmpty()) {
                    ...
                    $url = route('laporan_pilkada_serentak.show', ['laporan_pilkada_serentak' => $laporanBaru->id_laporan, 'from' => 'notification']);
            ...
            return redirect()->route('laporan_pilkada_serentak.index')
     ...
     */
    public function show(LaporanPilkadaSerentak $laporan_pilkada_serentak)
    {
        $laporan = $laporan_pilkada_serentak;
        $laporan->loadMissing('operator');

        return view('dashboard.laporan_pilkada_serentak.show', compact(
            'laporan',
        ));
    }
    ...
    {
        $laporan = LaporanPilkadaSerentak::findOrFail($id);
        return view('dashboard.laporan_pilkada_serentak.edit', compact('laporan'));
    }
    ...
    public function update(Request $request, string $id): RedirectResponse
    {
        ...
        try {
            ...
            if ($originalStatus === LaporanPilkadaSerentak::STATUS_REVISION) {
                $updateData['status_laporan'] = LaporanPilkadaSerentak::STATUS_PENDING;
                ...
                try {
                    ...
                    if ($pimpinanUsers->isNotEmpty()) {
                        ...
                        $url = route('laporan_pilkada_serentak.show', ['laporan_pilkada_serentak' => $laporan->id_laporan, 'from' => 'notification']);
    ...
    public function destroy(string $id)
    {
        try {
            ...
            foreach ($this->fileFields as $field) {
                $fileKey = "file_$field";
                $files = $laporan->$fileKey ?? []; // $casts di Model akan mengubahnya jadi array
